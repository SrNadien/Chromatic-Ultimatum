...
ServerEvents.tags('item', event => {
  ...
  for (let i = 0; i < tiers.length; i++) {
    ...
    event.add(`ultimate_utilities:farmland/${tierA}`, farmA.getId())
    if (farmB) {
      let tierB = farmB.getIdLocation().getPath().replace('_farmland', '')
      event.add(`ultimate_utilities:farmland/${tierA}`, `#ultimate_utilities:farmland/${tierB}`)
    } else {
      break
    }
  }
...
ServerEvents.recipes(event => {
  ...
  JsonIO.write('kubejs/server_scripts/cropInfo.json', JsonExport)
  ...
  if (Platform.isLoaded('botanypots')) {
    ...
    for (const seed of JsonExport.enabled) {
      if (!seenSeeds.includes(seed)) {
        ...
        event.custom({
          ...
          drops: drops
        }).id(`ultimate_utilities:botanypots/mysticalagriculture/${seed}`)
      }
    }
    // add crux 'soils'
    for (const block in crux) {
      ...
      event.custom({
        ...
        categories: [category],
        growthModifier: 1.0
      }).id(`ultimate_utilities:botanypots/mysticalagriculture/crux/${category}`)
    }
  ...
  if (Platform.isLoaded('thermal')) {
    JsonExport.enabled.forEach(cropName => {
      ...
      }).id(`ultimate_utilities:thermal/machines/insolator/mysticalagriculture/${cropName}`)
    })
  }
  ...
  if (Platform.isLoaded('immersiveengineering')) {
    JsonExport.enabled.forEach(cropName => {
      ...
      event.custom({
        ...
        soil: Ingredient.of(Crop.getCruxBlock() ?? `#ultimate_utilities:farmland/${Crop.getTier().getFarmland().getIdLocation().getPath().replace('_farmland', '')}`).toJson(),
        time: 250 + (750 * Crop.getTier().getValue()),
        render: {
          type: 'crop',
          block: Crop.getCropBlock().getId()
        }
      }).id(`ultimate_utilities:immersiveengineering/cloche/mysticalagriculture/${cropName}`)
    })
  }
})
